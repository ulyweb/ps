<# ============================================
 ...
 Updates: Added Hibernate Detection & Enable Button; maintained modular HTML export
    ...
    <div id="tab-reports" class="tab-pane">
      <div class="card" style="text-align:center; padding: 50px 20px; max-width:800px; margin:0 auto;">
        ...
        <div style="margin-top: 40px; padding-top: 30px; border-top: 1px dashed var(--border);">
            ...
            <p style="color:var(--text-muted); font-size:12px; margin-bottom:15px;">Select an existing log to launch directly in Excel on the user's screen.</p>
            <div style="display:flex; justify-content:center; gap:10px; align-items:center;">
                <select id="log-selector" class="settings-input" style="width:350px; margin-top:0; padding:10px;"></select>
                <button class="btn action-btn" style="padding:10px 15px; margin:0;" onclick="openSelectedLog()">&#128196; Open in Excel</button>
            </div>
...
<script>
  ...
  async function openSelectedLog() {
      ...
      if(!sel) return;
      document.getElementById('open-status').innerText = "Triggering Excel in remote session...";
      ...
      try {
          ...
          document.getElementById('open-status').innerText = "Launched " + sel;
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            elseif ($request.Url.AbsolutePath -eq "/api/capture" -and $request.HttpMethod -eq "POST") {
                ...
                try {
                    New-CaptureHtmlDashboard -Rows $csvRows -OutputPath $htmlPath -Days $days -GeneratedAt (Get-Date) | Out-Null
